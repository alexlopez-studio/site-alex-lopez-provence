...
const heroSignals = ['Estimation gratuite', TERRITORY, 'Sans engagement']
const estimationSteps = [
  { icon: Home, title: 'Décrivez votre bien', desc: 'Type, surface, adresse, état général.' },
  { icon: BarChart3, title: 'Analyse locale', desc: 'Lecture du village, du secteur et des facteurs de valeur.' },
...
const services = [
  { icon: BarChart3, title: 'Estimation', desc: 'Une première évaluation claire pour comprendre la valeur de votre bien avant de décider.', href: ESTIMATION_URL, cta: 'Estimer mon bien' },
  ...
  { icon: BookOpen, title: 'Guide Vendeur PAP', desc: '41 pages de méthodes, checklists imprimables A4 et conseils d’expert pour vendre seul au meilleur prix.', href: '/guide-vendeur', cta: 'Consulter le guide' },
...
function OutlineLink({ href, children, light = false }: { href: string; children: React.ReactNode; light?: boolean }) { return <Link href={href} className={'inline-flex items-center justify-center rounded-full border-2 px-7 py-4 text-sm font-bold transition-all hover:-translate-y-0.5 ' + (light ? 'border-white text-white hover:bg-white hover:text-foreground' : 'border-brand text-brand hover:bg-brand hover:text-white')}>{children}</Link> }
    ...
    <>
      <section className="relative overflow-hidden bg-[#f4f7f8] px-6 pb-16 pt-28 lg:min-h-screen lg:pb-24 lg:pt-36">
        ...
        <motion.div variants={stagger} initial="initial" animate="animate" className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1fr_0.9fr]">
          <div>
            <motion.div variants={fadeInUp} className="mb-6 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-foreground shadow-sm"><MapPin size={15} className="text-brand" /> {TERRITORY}</motion.div>
            ...
            <motion.div variants={fadeInUp} className="mt-9 flex flex-col gap-4 sm:flex-row"><PrimaryLink href={ESTIMATION_URL}>Estimer mon bien gratuitement <ArrowRight size={18} /></PrimaryLink><OutlineLink href="#secteur">Découvrir mon secteur</OutlineLink></motion.div>
          ...
          <motion.div variants={scaleIn} className="relative mx-auto flex min-h-[32rem] w-full max-w-[34rem] items-end justify-center lg:ml-auto">
            ...
            <Image src="/alexandre-lopez-no-background.png" alt="Alexandre Lopez, conseiller immobilier iad France en Provence Verte & Verdon" width={760} height={920} priority className="relative z-10 h-auto max-h-[35rem] w-auto object-contain drop-shadow-2xl" />
      ...
      <section id="estimation" className="bg-white px-6 py-20"><motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="mx-auto max-w-5xl text-center"><motion.p variants={fadeInUp} className="mb-4 text-sm font-bold uppercase tracking-[0.22em] text-brand">Estimation gratuite</motion.p><motion.h2 variants={fadeInUp} className="text-3xl font-bold leading-tight tracking-[-0.04em] text-foreground md:text-5xl">Quelle est la valeur de votre bien en Provence Verte & Verdon ?</motion.h2><motion.p variants={fadeInUp} className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-muted">Maison de village, bastide, villa avec piscine ou terrain : obtenez un premier repère fiable avant d’engager une vente ou une discussion.</motion.p><motion.div variants={scaleIn} className="mt-12 rounded-[2rem] bg-gradient-to-br from-[#f4f7f8] to-[#eaeef1] p-7 shadow-xl md:p-12"><div className="mb-10 grid gap-6 md:grid-cols-3">{estimationSteps.map((step) => { const Icon = step.icon; return <div key={step.title} className="text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-brand to-brand-hover text-white shadow-lg"><Icon size={28} /></div><h3 className="text-lg font-bold text-foreground">{step.title}</h3><p className="mt-2 text-sm leading-relaxed text-muted">{step.desc}</p></div> })}</div><PrimaryLink href={ESTIMATION_URL}>Lancer mon estimation gratuite <ArrowRight size={18} /></PrimaryLink><p className="mt-5 text-sm text-muted">Sans engagement · 100% gratuit · Premier résultat en quelques minutes</p></motion.div></motion.div></section>
      ...
      <section className="relative min-h-[34rem] overflow-hidden px-6 py-24 text-white md:min-h-[42rem] md:py-32">
        ...
        <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="relative mx-auto flex min-h-[24rem] max-w-7xl items-end md:min-h-[28rem]">
          <div className="max-w-3xl"><motion.p variants={fadeInUp} className="mb-4 inline-flex rounded-full bg-white/12 px-4 py-2 text-sm font-bold uppercase tracking-[0.18em] text-brand-light backdrop-blur">Cotignac · {TERRITORY}</motion.p><motion.h2 variants={fadeInUp} className="text-4xl font-bold leading-tight tracking-[-0.045em] md:text-6xl">Un projet immobilier ne se joue pas seulement sur un prix au m².</motion.h2><motion.p variants={fadeInUp} className="mt-6 max-w-2xl text-lg leading-relaxed text-white/82">En Provence Verte & Verdon, deux biens similaires peuvent avoir des valeurs très différentes selon le village, la rue, l’accès, la vue, l’état, le charme ou la demande locale.</motion.p></div>
        </motion.div>
      </section>

      <section id="secteur" className="bg-[#f4f7f8] px-6 py-20"><motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="mx-auto max-w-7xl"><motion.div variants={fadeInUp} className="mb-14 text-center"><p className="mb-4 text-sm font-bold uppercase tracking-[0.22em] text-brand">Mon secteur</p><h2 className="text-3xl font-bold tracking-[-0.04em] text-foreground md:text-5xl">Provence Verte & Verdon</h2><p className="mx-auto mt-5 max-w-3xl text-lg leading-relaxed text-muted">Une connaissance du territoire, de Brignoles aux portes du Verdon, pour vendre, acheter ou évaluer dans les meilleures conditions.</p></motion.div><motion.div variants={staggerFast} className="mb-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{sectorCards.map(([city, label]) => <motion.div key={city} variants={scaleIn} className="rounded-2xl bg-white p-6 text-center shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl"><div className="mb-2 text-2xl font-bold text-brand">{city}</div><p className="text-sm text-muted">{label}</p></motion.div>)}</motion.div><motion.div variants={fadeInUp} className="rounded-[2rem] bg-white p-8 shadow-sm md:p-10"><div className="grid items-center gap-8 lg:grid-cols-[1fr_0.75fr]"><div><h3 className="mb-4 text-2xl font-bold text-foreground">Je couvre également</h3><div className="flex flex-wrap gap-3">{communes.map((commune) => <span key={commune} className="rounded-full bg-[#f4f7f8] px-4 py-2 text-sm font-semibold text-muted">{commune}</span>)}</div></div><div><p className="mb-6 text-lg leading-relaxed text-muted">Votre commune n’apparaît pas ? Contactez-moi pour vérifier si je peux vous accompagner.</p><PrimaryLink href="/contact">Me contacter</PrimaryLink></div></div></motion.div></motion.div></section>

      <section className="relative min-h-[30rem] overflow-hidden px-6 py-20 text-white md:min-h-[36rem]"><Image src="/maison-bleue-cotignac.jpg" alt="Maison et ruelle à Cotignac en Provence Verte & Verdon" fill sizes="100vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-[#101828]/78 via-[#101828]/35 to-transparent" /><motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="relative mx-auto flex min-h-[22rem] max-w-7xl items-center"><motion.div variants={fadeInUp} className="max-w-2xl"><p className="mb-4 text-sm font-bold uppercase tracking-[0.22em] text-brand-light">Villages du secteur</p><h2 className="text-3xl font-bold tracking-[-0.04em] md:text-5xl">Des villages, des rues, des biens et des valeurs très différentes.</h2><p className="mt-5 text-lg leading-relaxed text-white/82">C’est ce niveau de lecture locale qui permet de mieux positionner un bien, préparer un achat ou identifier les points à vérifier.</p></motion.div></motion.div></section>

      <section id="services" className="bg-[#f4f7f8] px-6 py-20"><motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="mx-auto max-w-7xl"><motion.div variants={fadeInUp} className="mb-14 text-center"><p className="mb-4 text-sm font-bold uppercase tracking-[0.22em] text-brand">Mes services</p><h2 className="text-3xl font-bold tracking-[-0.04em] text-foreground md:text-5xl">Un accompagnement sur mesure</h2><p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-muted">Que vous souhaitiez vendre, acheter ou simplement vous renseigner, chaque étape doit être claire.</p></motion.div><motion.div variants={staggerFast} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{services.map((service) => { const Icon = service.icon; return <motion.div key={service.title} variants={scaleIn} className="rounded-2xl bg-white p-7 shadow-sm transition-all hover:-translate-y-2 hover:shadow-xl flex flex-col justify-between"><div className="mb-5 flex h-13 w-13 items-center justify-center rounded-xl bg-brand/10 text-brand"><Icon size={25} /></div><div><h3 className="text-xl font-bold text-foreground">{service.title}</h3><p className="mt-2.5 text-sm leading-relaxed text-muted">{service.desc}</p></div><Link href={service.href} className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-brand transition-all hover:gap-3">{service.cta} <ArrowRight size={15} /></Link></motion.div> })}</motion.div></motion.div></section>
      <section id="pourquoi" className="bg-white px-6 py-20"><motion.div variants={stagger} initial="initial" whileInView="animate" viewport={vpOnce} className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1fr_0.9fr]"><motion.div variants={fadeInUp}><p className="mb-4 text-sm font-bold uppercase tracking-[0.22em] text-brand">Pourquoi me choisir</p><h2 className="text-3xl font-bold leading-tight tracking-[-0.04em] text-foreground md:text-5xl">Un expert local, la force d’un réseau national</h2><p className="mt-6 text-lg leading-relaxed text-muted">Installé en Provence Verte & Verdon, je connais les nuances du territoire. Cette expertise locale, combinée à la puissance du réseau iad France, vous aide à avancer avec une vision plus juste.</p><div className="mt-9 space-y-6">{reasons.map(([title, desc]) => <div key={title} className="flex gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand/10 text-brand"><ShieldCheck size={22} /></div><div><h3 className="text-lg font-bold text-foreground">{title}</h3><p className="mt-1 text-sm leading-relaxed text-muted">{desc}</p></div></div>)}</div></motion.div><motion.div variants={staggerFast} className="grid grid-cols-2 gap-5">{[['30+', 'Communes couvertes'], ['iad', 'Réseau immobilier national'], ['7j/7', 'À votre écoute'], ['0€', 'Estimation gratuite']].map(([value, label]) => <motion.div key={label} variants={scaleIn} className="rounded-2xl bg-[#f4f7f8] p-7 text-center"><div className="mb-2 text-4xl font-bold text-brand md:text-5xl">{value}</div><p className="text-sm font-semibold text-muted">{label}</p></motion.div>)}</motion.div></motion.div></section>
