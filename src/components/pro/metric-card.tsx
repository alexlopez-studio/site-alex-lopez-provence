...
const toneClasses = {
  ...
  brand: 'bg-brand-light/35 text-foreground ring-brand/20',
...
function MetricCard({
  ...
  return (
    <article
      className={cn(
        'flex min-h-32 flex-col justify-between rounded-lg p-4 shadow-sm ring-1',
        toneClasses[tone],
        className
      )}
      {...props}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {Icon ? (
          <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-background/80 ring-1 ring-border/70">
      ...
      <div className="space-y-1">
        <div className="flex flex-wrap items-baseline gap-2">
          <p className="text-2xl font-semibold leading-none tracking-normal">
            {value}
          </p>
          {trend ? (
